...
struct item {
    ...
    float x;
    float y;
    float z; // Displacement value (from mouse for crafting/inventory/container items and from player for world items)
};

struct container {
    struct item container_items[8];
    int container_id;
    float x;
    float y;
...
void init(void) {
    ...
    world_items[2].x = 180;
    world_items[2].y = 180;
    world_items[2].item_id = 4;
    world_items[2].name = "mint";
